...
const StatCounter = ({ value, label, icon }: { value: number; label: string; icon: string }) => {
  ...
  return (
    <motion.div
      variants={bounceIn}
      className="game-card transform hover:scale-105 transition-all duration-300"
      whileHover="hover"
    >
      <motion.div
      ...
      <motion.span
        className="text-4xl font-gaming text-game-blue block"
        variants={neonPulse}
        initial="initial"
        animate="animate"
      >
      ...
      <p className="text-game-white/80 mt-2">{label}</p>
    </motion.div>
  );
};
...
const FeatureCard = ({ title, description, icon }: { title: string; description: string; icon: string }) => {
  return (
    ...
    >
      ...
      <motion.div
        variants={powerUpAnimation}
        className="text-4xl mb-4 text-game-blue"
      >
        {icon}
      </motion.div>
      <h3 className="text-2xl font-gaming mb-4 text-game-blue">{title}</h3>
      <p className="text-game-white/80 relative z-10">{description}</p>
        ...
        >
          ...
          <div className="min-h-screen flex items-center justify-center">
            ...
            >
              ...
              <motion.h1
                variants={fadeInDown}
                className="text-6xl md:text-8xl font-gaming font-bold mb-8"
              >
                <motion.span
                  variants={pixelateIn}
                ...
                <motion.span
                  ...
                  initial="initial"
                  animate="animate"
                  className="block text-game-red mt-4"
                >
                  Level Up Your Life
                </motion.span>
              </motion.h1>
              ...
              <motion.p
                variants={fadeInUp}
                className="text-xl md:text-3xl mb-12 text-game-white/90 font-gaming"
              >
              ...
              >
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="neon-button bg-game-red text-xl px-8 py-4 rounded-lg w-64 relative overflow-hidden group"
                >
                  <motion.span
                ...
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="neon-button bg-game-blue text-xl px-8 py-4 rounded-lg w-64 relative overflow-hidden group"
                >
                  <motion.span
                    variants={shakeAnimation}
                    className="relative z-10"
